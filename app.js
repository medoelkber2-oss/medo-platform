...
app.post('/forgot-password', async (req, res) => {
    ...
    if (!user) return res.render('forgot-password', { error: 'الإيميل غير مسجل', success: '', resetLink: '' });
...
app.get('/reset-password/:token', async (req, res) => {
    ...
    if (!user) return res.render('reset-password', { error: 'رابط غير صالح', success: '', token: null });
...
app.post('/reset-password/:token', async (req, res) => {
    ...
    const user = await User.findOne({ resetToken: req.params.token, resetTokenExpiry: { $gt: Date.now() } });
    if (!user) return res.render('reset-password', { error: 'رابط غير صالح', success: '', token: req.params.token });
...
app.post('/admin/add-admin', async (req, res) => {
    ...
    if (existingUser) {
        ...
        await User.create({ username, email, password: defaultPassword, isAdmin: true, courses: '{}' });
    }
    res.redirect('/admin#admins-section');
});

app.get('/admin/delete-admin/:id', async (req, res) => {
    if (!req.session.isAdmin || req.params.id === 'admin-main') return res.redirect('/admin');
...
app.post('/admin/edit-lecture/:courseId/:lecIndex', async (req, res) => {
    ...
    if (course) {
        course.lectures[parseInt(req.params.lecIndex)] = { title, vid };
        await course.save();
    }
    res.redirect('/admin');
});
...
app.get('/admin/delete-lecture/:courseId/:lecIndex', async (req, res) => {
    ...
    const course = await Course.findById(req.params.courseId);
    if (course) {
        course.lectures.splice(parseInt(req.params.lecIndex), 1);
        await course.save();
    }
    res.redirect('/admin');
});
